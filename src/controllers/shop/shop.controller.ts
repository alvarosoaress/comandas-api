...
import { type getShopByIdType, type createShopType } from '../../schema/shop.schema';
...
export async function getShopById (req: Request<getShopByIdType>, res: Response) {
  const { id } = req.params;

  const shopFound = await db.query.user.findFirst({
    where:
     and(
       eq(user.id, parseInt(id)),
       eq(user.role, 'shop')
     ),
